...
/**
 * Protobuf传输之客户端的案例
 * 客户端的出站处理流程中，Protobuf协议的编码过程如8-5所示，过程如下：
 * （1）先使用Netty内置的ProtobufEncoder，将Protobuf POJO对象编码成二进制的字节
 * 数组；
 * （2）然后，使用Netty内置的ProtobufVarint32LengthFieldPrepender编码器，加上
 * varint32格式的可变长度。Netty会将完成了编码后的Length+Content格式的二进制字节码发
 * 送到服务器端。
 */
